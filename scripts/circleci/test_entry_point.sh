...
if [ $((0 % CIRCLE_NODE_TOTAL)) -eq "$CIRCLE_NODE_INDEX" ]; then
  ...
  COMMANDS_TO_RUN+=('yarn test --runInBand --testResultsProcessor=jest-junit')
...
if [ $((1 % CIRCLE_NODE_TOTAL)) -eq "$CIRCLE_NODE_INDEX" ]; then
  COMMANDS_TO_RUN+=('yarn test-prod --runInBand --testResultsProcessor=jest-junit')
fi

if [ $((2 % CIRCLE_NODE_TOTAL)) -eq "$CIRCLE_NODE_INDEX" ]; then
  COMMANDS_TO_RUN+=('./scripts/circleci/build.sh')
  COMMANDS_TO_RUN+=('yarn test-build --runInBand --testResultsProcessor=jest-junit')
  COMMANDS_TO_RUN+=('yarn test-build-prod --runInBand --testResultsProcessor=jest-junit')
